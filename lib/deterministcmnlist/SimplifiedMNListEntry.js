...
SimplifiedMNListEntry.prototype.writeToBufferWriter = function (bufferWriter) {
  bufferWriter.write(this.toBuffer());
};
/**
...
SimplifiedMNListEntry.prototype.validate = function validate() {
  ...
  $.checkArgument(ipUtils.isIPV4(this.service), 'Expected service to be an ip address and port');
  $.checkArgument(isHexStringOfSize(this.keyIDOperator, PUBKEY_ID_SIZE * 2), 'Expected keyIDOperator to be a pubkey id');
  $.checkArgument(isHexStringOfSize(this.keyIDVoting, PUBKEY_ID_SIZE * 2), 'Expected keyIDOperator to be a pubkey id');
