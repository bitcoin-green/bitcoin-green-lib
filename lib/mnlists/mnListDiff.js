...
MnListDiff._fromBufferReader = function _fromBufferReader(br) {
  ...
  info.cbTx = Cbtx.fromBufferReader(br.read(38)); //todo check size
  ...
  info.mnList = []
  for (var i = 0; i < br.readVarintNum(); i++) {
    info.mnList.push(SmlEntry.fromBufferReader(br.read(91))); //todo check size
  }
