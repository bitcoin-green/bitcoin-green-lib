...
var SPECIAL_TRANSACTION_ACTIVATION_VERSION = 2;
...
Transaction.prototype.toBufferWriter = function(writer, noWitness) {
  ...
  if (this.isSpecialTransaction() && this.hasExtraPayload()) {
    var payload = this.extraPayload; //Payload.serializeToBuffer(this.extraPayload);
...
Transaction.prototype.fromBufferReader = function(reader) {
  ...
  console.log('--> reader.pos: ' + reader.pos*2);
  // Read legacy version
  ...
  console.log('tx version: ' + this.version)
  ...
  for (var i = 0; i < sizeTxOuts; i++) {
    this.outputs.push(Output.fromBufferReader(reader));
  }

  if (hasWitnesses) {
	console.log('--> witness.pos: ' + reader.pos*2);
  ...
  console.log('--> extraPayloadSize.pos: ' + reader.pos*2);
  console.log('this.isSpecialTransaction(): ' + this.isSpecialTransaction());
  if (this.isSpecialTransaction() && !reader.finished()) {

    var extraPayloadSize = reader.readVarintNum();
    console.log('extraPayloadSize: ' + extraPayloadSize);
    if (extraPayloadSize > 0) {
      var payloadBuffer = reader.read(extraPayloadSize);
      this.extraPayload = payloadBuffer;
...
Transaction.prototype.toObject = Transaction.prototype.toJSON = function toObject() {
  ...
  if (this.isSpecialTransaction()) {
    obj.type = this.type;
    obj.extraPayload = this.extraPayload; //.toString('hex'); //Payload.serializeToBuffer(this.extraPayload).toString('hex');
  }
  return obj;
};
...
Transaction.prototype.fromObject = function fromObject(arg) {
  ...
  if (transaction.type) {
    ...
    if (transaction.extraPayload) {
	  this.extraPayload = transaction.extraPayload;
...
Transaction.prototype.isSpecialTransaction = function() {
  ...
  console.log('SPECIAL_TRANSACTION_ACTIVATION_VERSION: ' + SPECIAL_TRANSACTION_ACTIVATION_VERSION);
  console.log('this.type: ' + this.type);
  console.log('!!this.type: ' + !!this.type);
  console.log('registeredTransactionTypes.TRANSACTION_NORMAL: ' + registeredTransactionTypes.TRANSACTION_NORMAL);
  return this.version >= SPECIAL_TRANSACTION_ACTIVATION_VERSION && this.type !== registeredTransactionTypes.TRANSACTION_NORMAL;
};
...
Transaction.prototype.getExtraPayloadSize = function getExtraPayloadSize() {
  return transaction.extraPayload.length; //Payload.serializeToBuffer(this.extraPayload).length;
};
